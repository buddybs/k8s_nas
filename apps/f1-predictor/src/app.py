...
def init_db():
    ...
    # Users table - user_id is the stable identity, pin for auth
    db.execute('''
    ...
    # Sessions table - multiple sessions per user (different browsers)
    db.execute('''
...
def get_current_user():
    ...
    db = get_db()
    # Join sessions to users to get user data
    user = db.execute('''
...
def signup():
    """Create new user account with PIN."""
    ...
    flash(f'Welcome, {username}! Your account is created.', 'success')
    return redirect(url_for('home'))
...
def login():
    ...
    if not user:
        flash('User not found. Create an account first.', 'error')
        return redirect(url_for('index'))
    ...
    # Create new session
    session_id = str(uuid.uuid4())
    db.execute('INSERT INTO sessions (session_id, user_id) VALUES (?, ?)', (session_id, user['user_id']))
    db.commit()
